...
const mintApprovalDeposit = new Command("mad")
        .description("Approve tokens for transfer")
        .option('--id <id>', "Token id", "0x1")
        .option('--recipient <address>', 'Destination recipient address', constants.deployerAddress)
        .option('--approvalFor <address>', 'Destination approvalFor address', constants.goerliERC721_HANDLER_ADDRESS)
        .option('--erc721Address <address>', 'ERC721 contract address', constants.goerliERC721_ADDRESS)
        .action(async function (args) {
            await setupParentArgs(args, args.parent.parent)
            const erc721Instance = new ethers.Contract(args.erc721Address, constants.ContractABIs.Erc721Mintable.abi, args.wallet);
            const tx = await erc721Instance.mint(args.wallet.address, ethers.utils.hexlify(args.id),"");
            console.log("pending: mint", args.id);
            await waitForTx(args.provider, tx.hash)

            const tx1 = await erc721Instance.approve(args.approvalFor,ethers.utils.hexlify(args.id));
            console.log("pending: approve", args.id);
            await waitForTx(args.provider, tx1.hash)

            // const bridgeInstance = new ethers.Contract(args.bridge, constants.ContractABIs.Bridge.abi, args.wallet);

        // const data = '0x' +
        //     ethers.utils.hexZeroPad(ethers.utils.hexlify(args.id), 32).substr(2) +  
        //     ethers.utils.hexZeroPad(ethers.utils.hexlify((args.recipient.length - 2) / 2), 32).substr(2) +       // len(recipientAddress) (32 bytes)
        //     ethers.utils.hexlify(args.recipient).substr(2)                // recipientAddress      (?? bytes)
        ...
        // Perform deposit
        // const tx3 = await bridgeInstance.deposit(
        //     args.dest, // destination chain id
        //     args.resourceId,
        //     data,
        //     {gasPrice: args.gasPrice, gasLimit: args.gasLimit});
        // await waitForTx(args.provider, tx3.hash)
...
erc721Cmd.addCommand(mintApprovalDeposit)
